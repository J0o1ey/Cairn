...
async function submit(claim: boolean) {
  if (!description.value.trim() || !projects.project) return;
  try {
    const actor = prefs.actorName;
    await projects.createIntent(projects.project.project.id, {
    ...
    ui.showToast(e instanceof Error ? e.message : '创建意图失败', 'error');
  }
}
</script>
...
<template>
  <ModalShell :show="show" @close="close">
    <div class="p-6">
      <h3 class="text-base font-semibold text-slate-700 mb-1">新建意图</h3>
      <div class="mb-4">
      ...
      <div class="flex justify-end gap-2 mt-5">
        ...
        <button
          ...
          class="px-5 py-2 text-sm border border-slate-200 text-slate-600 rounded-xl font-medium hover:bg-slate-50 transition disabled:opacity-30"
        >
          仅声明
        </button>
        <button
